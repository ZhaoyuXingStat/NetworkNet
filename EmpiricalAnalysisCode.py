...
# =============================================================================
# MODEL DEFINITIONS (These are correct and remain unchanged)
# =============================================================================
def estimate_mle_params(Y: np.ndarray, n_epochs: int = 1000, lr: float = 0.05, verbose: bool = False, device='cpu') -> tuple[np.ndarray, np.ndarray]:
    ...
    for epoch in range(n_epochs):
        ...
        lambda_ij = torch.exp(alpha.T + beta); mask = ~torch.eye(n, dtype=torch.bool, device=device)
...
class CountNetworkLassoNet(nn.Module):
    def __init__(self, input_dim, hidden_dims=(32, 16), dropout=0.1):
        ...
        for hidden_dim in hidden_dims[1:]:
            layers.extend([nn.Linear(prev_dim, hidden_dim), nn.ReLU(), nn.Dropout(dropout)]); prev_dim = hidden_dim
...
def _hierarchical_prox(model: nn.Module, lambda_: float, step_size: float, M: int):
    with torch.no_grad():
        ...
        for j in range(model.input_dim):
            theta_j, w_j = theta[0, j].item(), W1[:, j]; threshold = lambda_ * step_size
            if abs(theta_j) <= threshold: theta[0, j], W1[:, j] = 0.0, 0.0
            else:
...
class AlternatingCountNetworkModel:
    def __init__(self, input_dim: int, hidden_dims: tuple, lambda_l1_alpha: float,
        ...
        self.net_alpha = CountNetworkLassoNet(input_dim, hidden_dims).to(device); self.net_beta = CountNetworkLassoNet(input_dim, hidden_dims).to(device)
        self.lambda_l1_alpha, self.lambda_l1_beta = lambda_l1_alpha, lambda_l1_beta; self.alpha_estimates_, self.beta_estimates_ = None, None
    def _prepare_edge_data(self, Y: torch.Tensor):
        n = Y.shape[0]; r_idx = torch.arange(n, device=self.device).repeat_interleave(n)
        c_idx = torch.arange(n, device=self.device).repeat(n); off_diag = r_idx != c_idx
    ...
    def _update_alpha_network(self, X_t, y_v, r_idx, c_idx, fixed_beta, n_epochs, lr):
        opt = optim.Adam(self.net_alpha.parameters(), lr=lr); beta_e, sum_beta = fixed_beta[c_idx], torch.sum(fixed_beta)
        for _ in range(n_epochs):
            self.net_alpha.train(); opt.zero_grad(); alpha_all = self.net_alpha(X_t).squeeze(); alpha_e = alpha_all[r_idx]
            log_lambda = alpha_e + beta_e; p_loss = torch.mean(torch.exp(log_lambda) - y_v * log_lambda)
            l1 = self.lambda_l1_alpha * self.net_alpha.l1_regularization(); bal = self.gamma_balance * (torch.sum(alpha_all) - sum_beta)**2
            loss = p_loss + l1 + bal; loss.backward(); opt.step(); _hierarchical_prox(self.net_alpha, self.lambda_l1_alpha, lr, self.M)
    def _update_beta_network(self, X_t, y_v, r_idx, c_idx, fixed_alpha, n_epochs, lr):
        opt = optim.Adam(self.net_beta.parameters(), lr=lr); alpha_e, sum_alpha = fixed_alpha[r_idx], torch.sum(fixed_alpha)
        for _ in range(n_epochs):
            self.net_beta.train(); opt.zero_grad(); beta_all = self.net_beta(X_t).squeeze(); beta_e = beta_all[c_idx]
            log_lambda = alpha_e + beta_e; p_loss = torch.mean(torch.exp(log_lambda) - y_v * log_lambda)
            l1 = self.lambda_l1_beta * self.net_beta.l1_regularization(); bal = self.gamma_balance * (sum_alpha - torch.sum(beta_all))**2
            loss = p_loss + l1 + bal; loss.backward(); opt.step(); _hierarchical_prox(self.net_beta, self.lambda_l1_beta, lr, self.M)
    def fit(self, Y, X, alpha_init=None, beta_init=None, max_iters=10, n_epochs_per_iter=20, lr=0.01, tol=1e-4, verbose=False):
        ...
        for it in range(max_iters):
            ...
            a_chg = torch.norm(a_curr - a_old)/(torch.norm(a_old) + 1e-8); b_chg = torch.norm(b_curr - b_old)/(torch.norm(b_old) + 1e-8)
    ...
    def predict(self, X):
        ...
        with torch.no_grad(): self.net_alpha.eval(); self.net_beta.eval()
        return self.net_alpha(X_t).squeeze().cpu().detach().numpy(), self.net_beta(X_t).squeeze().cpu().detach().numpy()
    def get_selected_features(self): return self.net_alpha.get_selected_features(), self.net_beta.get_selected_features()

# ==================== 1. 数据加载和预处理 ====================
print("="*80); print("1. 加载数据"); print("="*80)
try:
...
if 'Author_uni_ID' not in features_df.columns:
    features_df.reset_index(inplace=True); features_df.rename(columns={features_df.columns[0]: 'Author_uni_ID'}, inplace=True)
authors_list = features_df['Author_uni_ID'].unique().tolist(); n_nodes = len(authors_list)
author_to_idx = {author_id: i for i, author_id in enumerate(authors_list)}; print(f"节点数: {n_nodes}")
numeric_cols = features_df.select_dtypes(include=np.number).columns.tolist()
feature_cols = [col for col in numeric_cols if col != 'Author_uni_ID']; n_features = len(feature_cols); print(f"特征维度: {n_features}")
X_df = features_df.set_index('Author_uni_ID').reindex(authors_list)[feature_cols]

# Fit the scaler ONCE and use it throughout
scaler = StandardScaler()
X = scaler.fit_transform(X_df.to_numpy(dtype=np.float32))

print("\n构建邻接矩阵..."); Y = np.zeros((n_nodes, n_nodes), dtype=np.float32)
...
def find_best_lambda_1d(parameter_type, target_k, fixed_lambda_other, fixed_model_params, fit_params, Y, X):
    """
    执行 1D 二分搜索以找到一个 lambda，
    同时将另一个 lambda 固定为 'fixed_lambda_other'。
    """
    ...
    for i in range(15): # 15 次二分搜索
        lambda_mid = (lambda_min + lambda_max) / 2
        
        # 根据搜索类型设置 alpha 和 beta 的 lambda
        ...
        # 二分搜索逻辑
        if n_sel > target_k + 1: # 选多了，需要增加惩罚
            lambda_min = lambda_mid
        else: # 选少了（或正好），尝试减少惩罚
            lambda_max = lambda_mid
            
        if abs(n_sel - target_k) <= 1: # 找到完美或接近的目标
            break
        if (lambda_max - lambda_min) < 1e-4: # 搜索范围太小
            break
...
### (已修改) 重命名: "parallel_worker" -> "run_analysis_for_architecture" ###
def run_analysis_for_architecture(config_tuple, Y_data, X_data, X_df_unscaled_data, scaler_obj, feature_cols_list, device):
    ...
    for search_iter in range(10): # 最多执行 5 轮交替搜索
...
if __name__ == '__main__':
    ...
    print("\n" + "="*80); print("3. 串行对比不同神经网络架构"); print("="*80)
    ...
    hidden_dims_options = {
        ...
        "Deep_Narrow (32, 16, 8, 8)": (64, 32, 32, 32),
        "Wide_Shallow (128, 64)": (128, 64)
    }
    
    # (partial worker 已删除)
    
    # (all_results = {} 已删除, 因为我们循环保存)
    ...
    for name, config in hidden_dims_options.items():
        ...
        output_filename = f"8090NEW_results_{safe_name}.csv"
        ...
        pd.set_option('display.width', 1000) # 让终端打印更宽
        print(f"\n--- {name} 结果预览 (Top 20) ---")
